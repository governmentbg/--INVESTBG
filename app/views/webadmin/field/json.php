...
$(function () {
    ...
    function collect() {
        // reindex first
        $('#<?= $this->e($field->getAttr("id")) ?>_form .json-form-rows .json-form-row').each(function (i) {
            $(this).find(':input[name]').each(function () {
                var name = $(this).attr('name').split('[');
                if (name[0] !== "<?= $this->e($field->getAttr('name')); ?>") {
                    return true;
                }
                $(this).attr('name', $(this).attr('name').replace(/\[n\d+\]/, '['+i+']'));
            });
        });
        ...
        $('#<?= $this->e($field->getAttr("id")) ?>')
            .val($('#<?= $this->e($field->getAttr("id")) ?>_form .json-form-rows .json-form-row').length ? JSON.stringify(val) : '')
            .prop('disabled', $('#<?= $this->e($field->getAttr("id")) ?>_form .json-form-rows .json-form-row').length > 0);
    }
